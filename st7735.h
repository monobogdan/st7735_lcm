/*
	ST7735 LCM library for single-board computers
	
	(C) 2023 Bogdan Nikolaev. All rights reserved.
	https://habr.com/ru/companies/timeweb/articles/753062/

	Usage:
		Export 10 and 20 pin in sysfs and set their direction to output.
		
		Init display (st7735Init)
		Prepare back buffer and send it using st7735Bitmap. It's assumed, that you send already prepared RGB565 128x160 buffer, i.e 40960 bytes.
*/
...
#define LCMLOGF(fmt, ...) printf("[%s]: " fmt "\n", &__FUNCTION__, __VA_ARGS__)
#define LCMLOG(fmt, ...) printf("[%s]: " fmt "\n", &__FUNCTION__)
...
void st7735Init()
{
    ...
    if(!fd)
    {
        LCMLOG("Failed to open spi device");
        return;
    }

    if(!dcFd)
    {
        LCMLOG("Failed to open DC GPIO");
        return;
    }
    ...
    st7735Reset();
    LCMLOG("Successfuly intiailized");
}
